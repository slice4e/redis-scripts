...
SHARD_COUNT = int(os.environ.get("SHARD_COUNT", "50"))
MEMORY_SIZE = int(1024 * 1024 * 1024 * SHARD_COUNT * 25)
MODULE_VERSION = os.environ.get("MODULE_VERSION", "2.8.9")
...
MT_MODE = ""
if WORKER_THREADS > 0:
    MT_MODE = "MT_MODE MT_MODE_FULL"

SOURCE_PORT = int(os.environ.get("SOURCE_PORT", "12000"))
SOURCE_FQDN = os.environ.get("SOURCE_FQDN", "192.168.2.22")
if not SOURCE_FQDN:
...
def create_sourcedb():
    req = {
        "name": "NODES-{}-SHARDS-{}-WORKER-THREADS-{}".format(
        ...
        "module_list": [
            {
                "module_args": "PARTITIONS AUTO WORKER_THREADS {} {}".format(
                    WORKER_THREADS, MT_MODE
                ),
