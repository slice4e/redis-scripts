...
if [ ! "$SKIP_SETUP" -eq 1 ]; then
    ...
    REDISEARCH_LIB=$REDISEARCH_PATH/bin/linux-x64-release-profile/search-community/redisearch.so
	...
	if [ "$USE_NUMACTL" -eq 1 ]; then
        	cmd="numactl -m ${NUMA_NODES} -N ${NUMA_NODES} $REDIS_PATH/src/redis-server $REDIS_PATH/redis.conf --PORT ${PORT} --logfile $REDIS_PATH/server.log --loadmodule $REDISEARCH_LIB WORKERS $REDISEARCH_WORKERS --save \"\" --protected-mode no --appendonly no "
        	echo -e $cmd
        	$cmd &
	else
        	cmd="$REDIS_PATH/src/redis-server $REDIS_PATH/redis.conf --PORT ${PORT} --logfile $REDIS_PATH/server.log --loadmodule $REDISEARCH_LIB WORKERS $REDISEARCH_WORKERS --save \"\" --protected-mode no --appendonly no"
        	echo -e $cmd
        	$cmd &
	fi
    fi
